...
export default class TextField extends PureComponent {
  static defaultProps = {
    ...
    tintColor: globalStyles.colors.blue.accent,
    textColor: globalStyles.colors.ink.primary,
    baseColor: globalStyles.colors.ink.lightest,
  ...
  renderAffixComponent(component, containerStyle, action) {
    if (!component) return null;
    const style = { width: 25, height: 25 };
    return (
      <SClickableArea onClick={action} style={style}>
  ...
  render() {
    ...
    let inputContainerStyle = {
      ...
      backgroundColor: globalStyles.colors.paper.dark,
    ...
    let inputStyle = {
      ...
      ...(props.multiline
        ? {
            ...
            height: 31 + height, //we need the height to span the entire container, to enable android to show the entire textinput when the keyboard is up
        ...
        : { height: 31 }),
      marginLeft: prefixComponent ? 5 : 0,
      marginRight: suffixComponent ? 5 : 0
    };
    ...
    let labelProps = {
      ...
      errored,
      restricted,
      style: prefixComponent
        ? { marginLeft: 40, ...labelTextStyle }
        : labelTextStyle
    };
    ...
    return (
      <View onClick={() => this.focus()} {...containerProps}>
        <Animated.View {...inputContainerProps}>
          {/* {disabled && <Line {...lineProps} />  remove Line on disabled to differentiate it from active text fields*/}

          <Label {...labelProps}>{label}</Label>

          <View style={styles.row}>
            {/* {this.renderAffix("prefix", active, focused)} */}
            {this.renderAffixComponent(
              prefixComponent,
              prefixContainerStyle,
              prefixAction
            )}
